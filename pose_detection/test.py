...
if __name__ == '__main__':
    ...
    for x in results[:3]:
        ...
        if x.shape[2]==20:
            ...
            print("→ Detected STRIDE 32 branch")
        if x.shape[2]==40:
            stride=16
            index=20*4*20*4
            print("→ Detected STRIDE 16 branch")
        if x.shape[2]==80:
            stride=8
            index=0
            print("→ Detected STRIDE 8 branch")
        print(f"Using stride={stride}, index={index}")
        feature=x.reshape(1,65,-1)
        output=process(feature,keypoints,index,x.shape[3],x.shape[2],stride)
        outputs=outputs+output
    predbox = NMS(outputs)
    print(f"Total raw boxes: {len(outputs)}")
    print(f"Total boxes after NMS: {len(predbox)}")
