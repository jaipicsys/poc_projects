...
def draw_velocity_bbox(frame, xyxy, velocity, thresh=400):
    try:
...
class RoomStateManager:
    ...
    def update(self, num_people: int, patient_posture: str, falling_detected: bool) -> str:
        ...
        # --- NEW RULE: CRITICAL State Clearance ---
        # Rule 0: Clear CRITICAL if attendant is present
        if self.state == "CRITICAL" and num_people > 1:
            ...
            print(f"[INFO] STATE CLEARED: CRITICAL → NORMAL (trigger=attendant_present)")
            self.state = "NORMAL" # Reset the state now
            self.pending_state = None
            self.pending_since = None
            return self.state # Exit early with the reset state
            
        # CRITICAL persists UNLESS cleared by the rule above
...
class FallDetector:
    ...
    def process_frame(self, frame) -> Tuple[List[dict], dict, Optional[np.ndarray]]:
        ...
        try:
            ...
            # inference error — return nothing (caller should handle)
            if self.debug:
        ...
        for r in results:
            ...
            # get boxes confidence array if present
            boxes_conf = None
            try:
            ...
            for det_idx in range(num_dets):
                ...
                if det_conf < self.bbox_conf_thresh:
                    continue
                ...
                # velocity from last queue entry
                prev_entry = None
                if self.person_states[pid]["queue"]:
                    prev_entry = self.person_states[pid]["queue"][-1]
                vel = self.compute_velocity(com, prev_entry, ts_ms)

                # compute downward displacement over window_ms
                ...
                # fall detection rule (velocity + down_disp)
                is_fall = False
                ...
                # prepare bbox for draw if available
                try:
                ...
                # log optionally
                if self.debug:
                    print(f"[DEBUG] [{ts_ms}] PID={pid} COM=({com[0]:.1f},{com[1]:.1f}) vel={vel:.2f}px/s down_disp={down_disp:.1f} posture={posture} mkc={mkc:.2f}")
                ...
                if is_fall:
                    ...
                    # gather seen persons for unattended logic later
                    seen_person_ids.add(pid)

                # update summary patient_posture (last seen posture)
                summary["patient_posture"] = posture
                summary["num_people"] += 1

                # optional per-detection unattended immediate alert is not appended here
                # unattended is decided after processing all detections per frame
...
class FallProcessor:
    def __init__(self, config: dict, camera_id: str, rtsp_url: str, on_done_callback=None, on_frame_callback=None):
        ...
        self.state_mgr = RoomStateManager(entry_delay_ms=config.get("low_entry_delay_ms", 5000))
