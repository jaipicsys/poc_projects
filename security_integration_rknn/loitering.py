...
class LoiteringProcessor:
    def __init__(self, config, camera_id: str, rtsp_url: str, on_done_callback=None):
        ...
        self.loiter_threshold = float(self.config.get("loitering_threshold_sec", 3))
        self.cooldown_seconds = float(self.config.get("loitering_cooldown_sec", 30))
        self.match_confidence = float(self.config.get("loitering_conf", 0.25))
        self.event_class = "loitering_detected"
        self.input_size = INPUT_SIZE
        self.track_state = {}

    def _point_in_zone_name(self, point):
        px, py = point
    ...
    def _save_snapshot(self, frame, frame_idx, track_id, zone_name=None, bbox=None, conf=None):
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        zpart = zone_name if zone_name else "zone"
        filename = f"loiter_{self.camera_id}_{zpart}_{track_id}_{frame_idx}_{ts}.jpg"
        path = os.path.join(self.snapshot_dir, filename)
        snap = frame.copy()
        if bbox:
            x1, y1, x2, y2 = map(int, bbox)
            cv2.rectangle(snap, (x1, y1), (x2, y2), (0, 0, 255), 2)
        label = f"{self.event_class} {conf:.2f}" if conf else self.event_class
        cv2.putText(snap, label, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2)
        cv2.imwrite(path, snap)
        return path

    def process_video(self):
        print(f"\n[INFO] Starting loitering processing for camera {self.camera_id}...")
        ...
        while True:
            ...
            if not cap.isOpened():
                print(f"[WARN] Loitering cam {self.camera_id} unavailable. Retrying in 2 minutes...")
            ...
            print(f"[INFO] Loitering cam {self.camera_id} connected.")
            ...
            try:
                while cap.isOpened():
                    ...
                    boxes_in_input = [b for b in boxes_in_input if b['class_id'] == 0]

                    convert_boxes_from_input_to_original(
                        boxes_in_input, self.input_size, self.input_size,
                        orig_w, orig_h, scale, pad_x, pad_y
                    )
                    final_boxes = NMS(boxes_in_input, NMS_IOU_THRESH)
                    dets_np = np.array([[b['x1'], b['y1'], b['x2'], b['y2'], b['conf']] for b in final_boxes], dtype=float) if final_boxes else np.empty((0, 5))
                    tracks = self.tracker.update(dets_np) if dets_np.shape[0] else []

                    now_ts = time.time()
                    for t in tracks:
                        x1, y1, x2, y2, track_id = map(int, t)
                        cx, cy = (x1 + x2)//2, (y1 + y2)//2
                        conf = next((b['conf'] for b in final_boxes if abs(b['x1']-x1)<3 and abs(b['y1']-y1)<3), None)
                        zone_name = self._point_in_zone_name((cx, cy))
                        state = self.track_state.setdefault(track_id, {"entry_times": {}, "last_logged": {}})

                        if zone_name:
                            entry = state["entry_times"].get(zone_name)
                            if entry is None:
                                state["entry_times"][zone_name] = now_ts
                            else:
                                duration = now_ts - entry
                                last_logged = state["last_logged"].get(zone_name, 0)
                                if duration >= self.loiter_threshold and (now_ts - last_logged) >= self.cooldown_seconds:
                                    snapshot_path = self._save_snapshot(frame, frame_idx, track_id, zone_name, [x1, y1, x2, y2], conf)
                                    insert_loitering_event_async(self.camera_id, frame_idx, track_id, self.event_class,
                                                                 round(duration, 2), zone_name, snapshot_path, datetime.now().isoformat())
                                    state["last_logged"][zone_name] = now_ts
                                    print(f"[LOITERING] {self.camera_id} | ID:{track_id} | zone:{zone_name} | dur:{duration:.1f}s")
                        else:
                            state["entry_times"].clear()
                    frame_idx += 1

            finally:
                cap.release()
                print(f"\n[INFO] Loitering processing done for {self.camera_id}")
