...
CONF_THRESH = 0.4
NMS_IOU_THRESH = 0.4
TRACKER_PARAMS = dict(max_age=90, min_hits=3, iou_threshold=0.3)
LOITERING_THRESHOLD = 3 # default time threshold in seconds
...
class LoiteringProcessor:
    def __init__(self, config: Optional[dict], camera_id: str, rtsp_url: str, on_done_callback=None):
        ...
        self.track_state = {}  # track_id -> {"entry_times": {zone_name: ts}, "last_logged": {zone_name: ts}}

        # Zones & thresholds
        self.zones = self.config.get("loitering_zones", {}).get(camera_id, [])
        self.time_threshold = float(self.config.get("loitering_time_threshold", LOITERING_THRESHOLD))
    ...
    def process_video(self):
        print(f"[INFO] Starting LoiteringProcessor for camera {self.camera_id}...")
        ...
        while True:
            ...
            while cap.isOpened():
                ...
                try:
                    ...
                    attrs = [None] * len(outputs)  # decode function requires attrs
                ...
                for t in tracks:
                    ...
                    if zone_name:
                        ...
                        if entry is None:
                            ...
                            if duration >= self.time_threshold and (now_ts - last_logged) >= self.cooldown_seconds:
                                snapshot_path = self._save_snapshot(frame, frame_idx, track_id, zone_name=zone_name, bbox=[x1,y1,x2,y2])
                                try:
                                    ...
                                    print(f"[LOITERING] {self.camera_id} | obj:{track_id} | zone:{zone_name} | duration:{duration:.1f}s | frame:{frame_idx}")
                                except Exception as e:
